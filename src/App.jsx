...
function App() {
  ...
  useEffect(() => {
    (async () => {
      try {
        const response = await fetch("https://book-api-lab2.vercel.app/");
  ...
  const addBook = async (newBook) => {
    try {
      const res = await fetch("https://book-api-lab2.vercel.app//books", {
        method: "POST",
  ...
  const updateBook = async (updatedBook) => {
    try {
      const res = await fetch(
        `https://book-api-lab2.vercel.app//books/${updatedBook.id}`,
        {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
  ...
  const deleteBook = async (id) => {
    try {
      const res = await fetch(`https://book-api-lab2.vercel.app//books/${id}`, {
        method: "DELETE",
      });
